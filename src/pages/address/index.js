...
const address = ({
  addressList, addressTotal, addressPage, addressPageSize,
  getAllRegion, postAddSubRegionsReq, putModifySubRegions, deleteSubRegions, savePagination
}) => {
    ...
    return [
      ...
      },
      {
        ...
        render: (text, record) => {
          return(
            <Space>
              <Button type="primary" onClick={() => handleOpenModifyModal(record)}>修改信息</Button>
              <Button type="danger" onClick={() => handleDeleteAddress(record)}>删除地址</Button>
            </Space>
          )
        }
      }
  ...
  return (
    <Card>
      <div>
        <Button style={{ margin: 10 }} type="primary" onClick={handleOpenCreateModal}>创建地址</Button>
      </div>
      <Table
        scroll={{ x: true }}
      ...
      >
        <Form form={form} preserve={false}>
          ...
          <Form.Item label="编号" name="postalCode" rules={
            [

...
const { Option } = Select;
const freight = ({ freightList, freightTotal, freightPage, freightPageSize, 
  postDefineShopFreightModel, getShopFreightModel, putModifyFreightModel, deleteFreightModel, savePagination
}) => {
    ...
    return [
      ...
      },
      {
        ...
        render: (text, record) => {
          ...
          return (
            <Space>
              <Button type="primary" onClick={() => handleModifyFreight(record)}>
                修改活动信息
              </Button>
              <Button type="default" onClick={() => history.push(`/freight/${id}`)}>查看详情</Button>
  ...
  return (
    <Card>
      <div style={{ margin: 10 }}>
        <Button type="primary" onClick={handleCreateFreight}>创建运费模板</Button>
      </div>
      <Table
        scroll={{ x: true }}
      ...
      >
        <Form form={form}>
          ...
          <Form.Item label="单元" name="unit" required
            rules={
              [
                { required: true, message: '请输入价格'}
              ]

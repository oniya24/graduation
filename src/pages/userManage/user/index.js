...
const userManage_user = ({ 
  userList, userTotal, userPage, userPageSize,
  getAllUsers, putBanUserByShop, putReleaseUserByShop, savePagination
 }) => {
    ...
    return [
      ...
      },
      {
        ...
        render: (text, record) => {
          return(
            <Space>
              <Button type="danger" onClick={() => handleBanUser(record)}>封禁用户</Button>
              <Button type="danger" onClick={() => handleReleaseUser(record)}>解禁用户</Button>
            </Space>
          )
        }
      }

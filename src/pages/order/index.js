...
import { Card, Table, Button, Tooltip, Space, Form, Input, Tag, Modal, DatePicker } from 'antd';
import {
  mapStateToProps,
  mapDispatchToProps,
} from '@/models/Order';
...
const userManage_comment = ({ 
  orderList, orderTotal, orderPage,
  orderPageSize, orderDetail,
  getAllOrder, getOrderById, 
  putModifyOrder, deleteOrderById, putDeliverOrder, savePagination
 }) => {
        ...
        render: (text, record) => {
          return(
            <Space>
              ...
              <Button type="danger" onClick={() => handleDeleteOrder(record)}>取消订单</Button>
            </Space>
          )
        }
      }
    ...
    <Card>
      ...
      >
        ...
        }
        {
          ...
          <Form
            size="small"
            layout="inline" onFinish={handleDeliverSubmit}
          >
            <Form.Item label="商家发货编号" name="freightSn">
              <Space>
                <Input/>
                <Button type="primary" htmlType="submit">提交单号</Button>
              </Space>
            </Form.Item>
          </Form> : null
        }

...
const advertise = ({ 
  adverList, adverTotal, adverPage, adverPageSize,
  putDefaultAdvertise, postUploadImg, putOnshelvesAdvertise, putOffshelvesAdvertise, putAuditAdvertise,
  putModifyAdvertise, deleteAdvertise, getAllSegmentsAdvertise, postCreateSegmentsAdvertise, savePagination
}) => {
        ...
        render: (text, record) => {
          ...
          return (
            <Space>
              ...
              <Button type="ghost" onClick={() => handleAdverDetail(record)}>查看详情</Button>
    ...
    <Card>
      <Card style={{ margin: 10 }}>
        ...
        >
          ...
          <Form.Item>
            <Space>
              <Button type="primary" htmlType="submit">当前时段创建广告</Button>
              <Button type="primary" onClick={handleCreateAdver}>当前时段创建广告</Button>
            </Space>
          </Form.Item>
        </Form>
      </Card>
      ...
      >
        <Form form={form}>
          ...
          <Form.Item label="活动名" name="content" required
            rules={
              [
          ...
          <Form.Item label="weight" name="weight" required
            rules={
              [
                { required: true, message: '请输入weight'}
              ]
            }
          >
            <Input type="number" />
          </Form.Item>
          <Form.Item label="开始时间" name="beginDate" required
            rules={
              [
                { required: true, message: '请输入beginDate'}
              ]
            }
          >
            <DatePicker/>
          </Form.Item>
          <Form.Item label="结束时间" name="endDate" required
            rules={
              [
                { required: true, message: '请输入endDate'}
              ]
            }
          >
            <DatePicker/>
      ...
      <Modal visible={adverDetailVisible} 
        title="广告状态"
        onCancel={() => setAdverDetailVisible(false)}
      >
        <Space>
            <span>活动状态</span>
            <Button type="primary" onClick={handleAudit}>审核通过</Button>
            <Button type="primary" onClick={handleSetDefault}>设置为默认活动</Button>
            <Button type="primary" onClick={handleOnShelves}>上架活动</Button>
            <Button type="primary" onClick={handleOffShelves}>下架活动</Button>
          </Space>
      </Modal>
    </Card>
  );

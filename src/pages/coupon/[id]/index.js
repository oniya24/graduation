...
import { Card, Table, Button, Tooltip, Space, Form, DatePicker, Modal, Input, PageHeader } from 'antd';
import {
  mapStateToProps,
  mapDispatchToProps,
} from '@/models/Coupon';
...
const coupon_detail = ({
  couponDetail, couponSkusList, couponSkusTotal, couponSkusPage, couponSkusPageSize,
  postModifyActivityRegion, deleteCouponSkusRegion, getCouponActivitySkusById, 
  getCouponActivityById, putOnshelvesActivity, putOffshelvesActivity, saveCouponSkusPagination
}) => {
    ...
    return [
      ...
      },
      {
        ...
        render: (text, record) => {
          ...
          return (
            <Space>
              <Button type="danger" onClick={() => handledeleteSkusFromCoupon(record)}>
                删除活动
              </Button>
            </Space>
          );
  ...
  return (
    ...
    >
      ...
      <Card title="活动商品" size="small">
        <div style={{ margin: 10 }}>
          <Button type="primary" onClick={handleAddSku2Coupon}>添加商品</Button>
        </div>
        <Table
          scroll={{ x: true }}

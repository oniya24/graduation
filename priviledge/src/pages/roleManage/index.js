...
const RoleManage = ({ 
  // 数据
  priList, roleList, rolePriList,
  roleTotal, rolePage, rolePageSize,
  // 方法
  ...
  const columns = useMemo(( )=> {
    return [
      ...
      },
      {
        ...
        render: (text, record) => {
          return (
            <Button type="primary" onClick={() => openPriModal(record)}>编辑权利信息</Button>
          )
        }
      },
      {
        ...
        render: (text, record) => {
          return (
            <Space>
              <Button type="primary" onClick={() => openUpdateRoleModal(record) }>修改角色</Button>
              <Button type="danger" onClick={() => deleteRole(record) }>删除角色</Button>
            </Space>
          )
        }
      },
  ...
  return (
    <Card>
      <div>
        <Button style={{ margin: 10 }} type="primary" onClick={openCreateRoleModal}>新建角色</Button>
      </div>
      <Table 
        rowKey={record => record.dataIndex}
        pagination={pagination(roleTotal,savePagination)} columns={columns} 
        dataSource={roleList}></Table>
      <Modal visible={roleModalVisible} destroyOnClose
        onOk={() => { roleStatus === 0 ? submitCreateRole() : submitUpdateRole() } } 
        onCancel={() => setRoleModalVisible(false)}>
        <Form form={form1} preserve={false}>
          {
            roleStatus === 0 ? 
            null : <Form.Item label="ID" name="id">
      ...
      <Modal visible={priModalVisible} destroyOnClose
        footer={[]}
      ...
      >
        ...
        <div style={{display:'flex', justifyContent:'flex-end'}}>
          ...
          <Button type="primary" onClick={addPriToRole}>增加权利</Button>
        </div>

...
import { Card, Form, Input, Select, Table, 
  Space, Button, Modal, Drawer, Descriptions, Divider } from 'antd';
import { mapStateToProps, mapDispatchToProps } from '@/models/AdminManage';
import pagination from '@/util/pagination';
import { deleteRoleByDidReq } from './../../service/RoleManage';
const { Option } = Select;
const layout = {
  labelCol: { span: 4 },
  wrapperCol: { span: 16 },
...
const AdminManage = ({ 
  // 数据
  adminList,adminInfo,
  adminRoleList, allRoleList,
  adminTotal, adminPage, adminPageSize,
  // 方法
  getAllAdmin, getAdminById,
  updateAdminById, deleteAdminById,
  approveAdminById, forbidAdminById, releaseAdminById,savePagination, 
  getAllRoleByDid, getRoleByDid, cancelRoleFromAdmin, addRoleFromAdmin
}) => {
  ...
  const submitAdminForm = () => {
    form.validateFields(["id","name","avatar","mobile","email"]).then((val) => {
      updateAdminById({
        ...val,
        did: depart_id,
      })
  ...
  const columns = useMemo(( )=> {
    return [
      ...
      },
      {
        ...
        render: (text, record) => {
          return (
            <Button type="primary" onClick={() => openRoleModal(record)}>编辑角色信息</Button>
          )
        }
      },
      {
        ...
        render: (text, record) => {
          return (
            <Space>
              <Button type="default" onClick={() => openAdminDrawer(record) }>查看详情</Button>
              <Button type="primary" onClick={() => openAdminModal(record) }>修改信息</Button>
              <Button type="danger" onClick={() => deleteAdmin(record) }>删除用户</Button>
            </Space>
          )
        }
      },
  ...
  return (
    <Card>
      ...
      <Table 
        ...
        pagination={pagination(adminTotal,savePagination)} columns={columns} 
        dataSource={adminList}></Table>
      <Modal visible={modalAdminVisible} destroyOnClose width={"50vw"}
        onCancel={closeAdminModal} onOk={submitAdminForm}>
        ...
        <Card title="基本信息">
          <Form {...layout}
            ...
            <Form.Item name="status" label="status">
              <Space>
                <Button type="danger" size="small" onClick={() => forbidAdminById({did:depart_id,id: adminInfo.id}) }>禁止用户</Button>
                <Button type="danger" size="small" onClick={() => releaseAdminById({did:depart_id,id: adminInfo.id}) }>释放用户</Button>
                <Button type="danger" size="small" onClick={() => approveAdminById({did:depart_id,id: adminInfo.id}) }>认证用户</Button>
              </Space>
            </Form.Item>
          </Form>
        </Card>
      </Modal>
      <Modal visible={modalRoleVisible}
      ...
      >
              ...
              <div key={"role"+index}>
                <div style={{display:'flex', justifyContent:'space-around'}}>
                  ...
                  <Button size="small" type="danger" 
                    onClick={() => deleteRole(item.role.id)}>删除当前角色</Button>
                </div>
                <Divider></Divider>
              </div>
            )
        ...
        <div style={{display:'flex', justifyContent:'flex-end'}}>
          ...
          <Button type="primary" size="small" onClick={ () => addRole(newRoleId) }>新增角色</Button>
        </div>
        </Card>
      </Modal>
      <Drawer 
        ...
        <Descriptions colon={true} column={1} >
          {
            Object.keys(adminInfo).map((key) =>{
              return (
              <Descriptions.Item label={key} key={key}>{adminInfo[key]}</Descriptions.Item>
              )
            })
          }
        </Descriptions>

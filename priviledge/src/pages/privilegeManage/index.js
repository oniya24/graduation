...
const { Option } = Select;
const Privilege = ({ priList, priPage, priPageSize, priTotal,
  getPriList, updatePriInfo, savePagination}) => {
    ...
    return [
      ...
      },
      {
        ...
        render: (text, record) => {
          return (
            <Space>
              <Button type="primary" onClick={() => openModal(record) }>修改权限</Button>
            </Space>
          )
        }
      }
  ...
  return (
    <Card>
      ...
      <Table 
        ...
        pagination={pagination(priTotal, savePagination)} columns={columns} 
        dataSource={priList}></Table>
      <Modal 
        destroyOnClose={true}
        visible={modalVisible} closable={false}
        cancelText={"取消"} onCancel={closeModal}
        
        okText={"修改"} onOk={submitForm}>
        <Form 
          form={form}  
          preserve={false}

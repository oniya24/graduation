...
// 查询某个depart下的所有admin
export const getAllAdminReq = ({did, ...params}: { did:number, params: any}) => {
...
// 通过id对admin进行更新
export const updateAdminByIdReq = ({did, id, ...params}: { did:number, id: number, params: any}) => {
  return request(`shops/${did}/adminusers/${id}`,
  {
    method: 'put',
    data: params
...
// 通过id删除某个admin
export const deleteAdminByIdReq = ({did, id}: { did:number, id: number}) => {
  return request(`shops/${did}/adminusers/${id}`,
  {
    method: 'delete',
  })
...
// 释放用户
export const releaseAdminByIdReq = ({did,id}:{id: number, did: number}) => {
...
// 禁止用户
export const forbidAdminByIdReq = ({did,id}:{id: number, did: number}) => {
...
// 为某个id添加角色
export const addRoleFromAdminReq = ({did,userid,roleid}:{did:number,userid:number,roleid:number}) => {
...
// 删除某个用户的角色
export const cancelRoleFromAdminReq = ({did,userid,roleid}:{did:number,userid:number,roleid:number}) => {
  return request(`shops/${did}/adminusers/${userid}/roles/${roleid}`,{
    method: 'delete'
  })
}
